...
import time
import os
# module webvtt only gets the line-captions, not individual words

# youtube-dl --skip-download --write-auto-sub 'https://www.youtube.com/watch?v=CrL5tqFczPE'
...
# def download_clips(regex, video_url):
#     with open("844.en.vtt") as f:
#         for interval in get_time_interval(r"but", f.read()):
#             download_interval()
...
def download_intervals(video_id, intervals):
    for start, stop in intervals:
        ...
        filename = f"clips/{video_id}s+{start}.mp4"
        if not os.path.isfile(filename):
            subprocess.run(['bash', 'download_clip.bash', video_id, start, str(duration), filename])
...
def download_clips(regex, video_ids, dry=False):
    for video_id, subtitle in download_subtitles(video_ids):
        ...
        # Delay between videos to respect the server
        time.sleep(2)
